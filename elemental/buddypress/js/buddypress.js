...
jq( document ).ready(
	function() {
		...
		jq( '#activity-filter-select select' ).change(
			function() {
		...
		jq( 'div.activity' ).on(
			...
			function(event) {
				...
				if ( target.parent().hasClass( 'load-more' ) ) {
					...
					jq( '.activity-list li.just-posted' ).each(
						function(){
		...
		jq( 'div.activity' ).on(
			...
			function(event) {
				...
				if ( target.parent().hasClass( 'show-all' ) ) {
					...
					setTimeout(
						function() {
							target.parent().parent().children( 'li' ).fadeIn(
								200,
								function() {
		...
		jq( '.dir-search, .groups-members-search' ).on(
			'click',
			function(event) {
		...
		jq( '.field-visibility-settings-close' ).on(
			'click',
			function( event ) {
				event.preventDefault();
		...
		jq( '#profile-edit-form input:not(:submit), #profile-edit-form textarea, #profile-edit-form select, #signup_form input:not(:submit), #signup_form textarea, #signup_form select' ).change(
			function() {
				var shouldconfirm = true;

				jq( '#profile-edit-form input:submit, #signup_form input:submit' ).on(
					'click',
					function() {
						shouldconfirm = false;
					}
		...
		jq( '#friend-list a.accept, #friend-list a.reject' ).on(
			'click',
			function() {
				var button = jq( this ),
		...
		jq( '#members-dir-list, #members-group-list, #item-header' ).on(
			'click',
			'.friendship-button a',
			function() {
		...
		jq( 'body.messages #item-body div.messages' ).on(
			'change',
			'#message-type-select',
			function() {
		...
		jq( '#select-all-messages' ).click(
			function(event) {
				if ( this.checked ) {
					jq( '.message-check' ).each(
						function() {
							this.checked = true;
						}
					);
		...
		jq( '#message-threads td.bulk-select-check :checkbox' ).on(
			'change',
			function() {
				var box = jq( this ),
		...
		jq( '#select-all-notifications' ).click(
			function(event) {
				if ( this.checked ) {
					jq( '.notification-check' ).each(
						function() {
							this.checked = true;
						}
					);
		...
		jq( '#select-all-invitations' ).on(
			'click',
			function() {
				if ( this.checked ) {
					jq( '.invitation-check' ).each(
						function() {
							this.checked = true;
						}
					);
		...
		jq( '#wp-admin-bar ul.main-nav li, #nav li' ).mouseover(
			function() {
				jq( this ).addClass( 'sfhover' );
			}
		);

		jq( '#wp-admin-bar ul.main-nav li, #nav li' ).mouseout(
			function() {
		...
		jq( '#wp-admin-bar-logout, a.logout' ).on(
			'click',
			function() {
				jq.removeCookie(
					'bp-activity-scope',

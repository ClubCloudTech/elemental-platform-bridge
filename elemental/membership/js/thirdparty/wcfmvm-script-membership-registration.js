/**
 * Onboarding JS for Site Onboard Wizard for Site.
 *
 * Originally by WC Lovers and WCFM Mememberships plugin
 * Modified by ClubCloud for Elemental usage inside of Ajax
 *
 * @package membership/js/thirdparty/wcfmvm-script-membership-registration.js
 */
jQuery( document ).ready(
	function($) {

		var init = function() {
			console.log( 'ajaxfire' );
		...
		function unwrapSelect() {
			$( '#wcfm-main-contentainer' ).find( 'input[type="checkbox"]' ).each(
				function() {
			...
			$( '#wcfm-main-contentainer' ).find( 'select' ).each(
				function() {
					if ($( this ).parent().is( "span" )) {
						$( this ).unwrap( "span" );
					}
		...
		function setStateBoxforCountry(countryBox) {
			...
			if (states[country]) {
				if ($.isEmptyObject( states[country] )) {
					countryBox.parent().find( '.wcfmvm_state_to_select' ).each(
						function() {
							$statebox          = $( this );

jQuery(document).ready(function($) {
    ...
    function initready() {
        ...
        if ($('.wcfmmp-store-search-form').length > 0) {
            ...
            jQuery(document).on('keyup', '.wcfmmp-store-search-form', function() {
                refreshStoreList();
            });
            ...
            jQuery(document).on('input', '#search', function(e) {
                refreshStoreList();
            });

            // Custom Search Box
            jQuery(document).on('input', '.wcfm-search-field', function(e) {
                refreshStoreList();
            });

            // Category Filter
            jQuery(document).on('change', '#wcfmmp_store_category', function() {
                refreshStoreList();
            });

            // Custom Search Frop Down
            jQuery(document).on('change', '.wcfm-custom-search-select-field', function() {
                refreshStoreList();
            });
            ...
            // State Filter
            jQuery(document).on('change', '#wcfmmp_store_state', function() {
                refreshStoreList();
            });

            // State Filter
            jQuery(document).on('keyup', '#wcfmmp_store_state', function() {
                refreshStoreList();
            });
            ...
            if ($('#wcfmmp_radius_addr').length > 0) {
                ...
                jQuery(document).on('input', '#wcfmmp_radius_range', function() {
    ...
    initready();
    initready($(document));
    window.wcfm_script_init = initready;
});
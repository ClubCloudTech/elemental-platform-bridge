...
class UserHelpers {
	...
	public function update_docvault_menu_cp_setting( array $input ): array {
		$input_add = ' 
		<td>
		<span>' . esc_html__( 'Document Vault Landing Page URL', 'elemental' ) . '</span>
		</td>
		<td>
		<input type="string" size="64"
		class="elemental-main-button-enabled elemental-maintenance-setting"
		...
		value="' . get_option( self::DOCVAULT_MENU_CP_SETTING ) . '">
			<i class="elemental-dashicons elemental-icons dashicons-editor-help" title="' . \esc_html__( ' The URL you want document notifications to land at <br>(used in user menu profile shortcode)', 'elemental' ) . '"></i>
		</td>';
		\array_push( $input, $input_add );
		return $input;
	}
